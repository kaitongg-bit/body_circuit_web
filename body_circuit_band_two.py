"""
Body Circuit Band - Two Person Version
Interactive music installation for 2 people using YOLOv8 Pose.
Two people form a circuit by holding both hands.
"""
...
class CircuitDetectorTwo:
    """Circuit closure detector for 2 people with asymmetric debouncing"""

    def __init__(self, distance_threshold: float = 0.3, debounce_frames_close: int = 8, debounce_frames_open: int = 2):
    ...
    def check_circuit(self, person_a: Person, person_b: Person) -> Tuple[bool, float, List[float]]:
        """
        Check if circuit is closed for 2 people
        Circuit: A right hand <-> B left hand, A left hand <-> B right hand
        Returns: (is_closed, average_distance, [d1, d2])
        """
        d1 = self.calculate_distance(person_a.right_wrist, person_b.left_wrist)
        d2 = self.calculate_distance(person_a.left_wrist, person_b.right_wrist)

        distances = [d1, d2]
        d_avg = sum(distances) / 2

        all_close = all(d < self.distance_threshold for d in distances)

        return all_close, d_avg, distances

    def update(self, person_a: Person, person_b: Person) -> Tuple[bool, float, List[float]]:
        """
        Update state with asymmetric debouncing (slow to close, fast to open)
        Returns: (current_stable_state, average_distance, [d1, d2])
        """
        instant_closed, d_avg, distances = self.check_circuit(person_a, person_b)
...
class VisualFeedback:
    """Visual feedback rendering for 2 people"""
    ...
    @staticmethod
    def draw_connections(frame, person_a: Person, person_b: Person,
                        distances: List[float], circuit_closed: bool,
                        frame_width: int, frame_height: int, threshold: float):
        """Draw connection lines for 2 people"""
        connections = [
            (person_a.right_wrist, person_b.left_wrist, distances[0], "A-right <-> B-left"),
            (person_a.left_wrist, person_b.right_wrist, distances[1], "A-left <-> B-right")
        ]
        ...
        for (p1, p2, dist, label) in connections:
            ...
            cv2.putText(frame, f"{dist:.3f}", (mid_x - 30, mid_y - 10),
...
class BodyCircuitBandTwo:
    """Body Circuit Band for 2 people"""

    def __init__(self, drum_path: str, bass_path: str, harmony_path: str, use_yolo: bool = True):
        self.use_yolo = use_yolo and YOLO_AVAILABLE
        if self.use_yolo:
            print("Using YOLOv8-Pose for 2-person detection")
            self.model = YOLO('yolov8n-pose.pt')
        else:
            print("Using simplified detection mode (demo only)")
            self.model = None

        self.circuit_detector = CircuitDetectorTwo(distance_threshold=0.3, debounce_frames_close=8, debounce_frames_open=2)
    ...
    def run(self, camera_index: int = 0):
        ...
        print("Body Circuit Band (2 People) Started")
        print("Note: Need 2 people to form a circuit by holding both hands")
        print("   - Person A right hand <-> Person B left hand")
        print("   - Person A left hand <-> Person B right hand")
        ...
        try:
            while cap.isOpened():
                ...
                if len(persons) >= 2:
                    ...
                    # Debug output
                    print(f"Distances: A-right<->B-left={distances[0]:.3f}, A-left<->B-right={distances[1]:.3f}, avg={d_avg:.3f}")
                    ...
                    cv2.putText(frame, f"Detected: {len(persons)}/2 people", (20, 50),
                               cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
                    cv2.putText(frame, "Need 2 people to start!", (20, 100),
                               cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 2)

                cv2.imshow('Body Circuit Band - 2 People', frame)
...
def main():
    ...
    band = BodyCircuitBandTwo(drum_path, bass_path, harmony_path, use_yolo=True)
